...
export const generateRecoveryId = async (email: string, username: string): Promise<{ original: string, hashed: string }> => {
...
export const buildPasswordRecoveryEmailTemplate = (username: string, publicAccountRecoveryId: string): string => {
    ...
    return ACCOUNT_RECOVERY_EMAIL_TEMPLATE.replaceAll('{{username}}', username).replaceAll('{{accountRecoveryLink}}', accountRecoveryLink.href);
};
