...
export const enqueueUploadedPostImage = async (
	...
	console.log(
		`Response after enqueueing post image for classification: ${JSON.stringify(response)} for postImageInput: ${rawMessageStream}`,
	);
	return response.$metadata.httpStatusCode === 200;
};

export const enqueueBatchUploadedPostImages = async (
	postImageInputs: TPostImageSqsMessage[],
): Promise<boolean> => {
