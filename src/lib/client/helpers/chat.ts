...
export class ChatManager {
    ...
    constructor(pingIntervalMs: number = DEFAULT_PING_INTERVAL_MS, reconnectIntervalMs: number = DEFAULT_RECONNECT_INTERVAL_MS) {
    ...
    registerIntervals() {
        ...
        const socketReconnectIntervalId = setInterval(() => {
            if (this.socket.readyState === WebSocket.CLOSED) {
                console.log('attempting to restablish socket connection');
                this.socket = new WebSocket(DEXBOORU_CORE_API_CHAT_WS_URL);
            }
